...
function MissionPage() {
  ...
  // Choose content track based on mode
  const track = advanced ? deep?.advanced : deep?.beginner;
  const whatParas: string[] | undefined = track?.what ?? deep?.definition;
  const quizQs = advanced
    ? (deep?.quizHard ?? m.quiz)
    : (deep?.quizEasy ?? m.quiz.slice(0, 3));
  const passThreshold = advanced ? 0.6 : 0.5;

  const onQuizDoneAdj = (score: number) => {
    completeMission(slug, m.xp, score, score >= passThreshold ? m.badge?.slug : undefined);
    setCompleted(true);
  };
    ...
    <div className="max-w-5xl mx-auto p-4 md:p-12 space-y-6 animate-fade-in">
      ...
      <section>
        ...
        {whatParas ? (
          <div className="space-y-2">
            {whatParas.map((p, i) => (
              <p key={i} className="font-mono text-sm md:text-base leading-relaxed">{p}</p>
            ))}
          </div>
        ...
        {!advanced && deep?.beginner?.analogy && (
          <div className="mt-3 brutal-border bg-acid p-3 font-mono text-sm">
            <span className="font-bold uppercase text-xs">Think of it like →</span> {deep.beginner.analogy}
          </div>
        )}
        {!advanced && deep?.beginner?.why && (
          <div className="mt-3 brutal-border bg-volt text-bone p-3">
            ...
            <ul className="space-y-1 font-mono text-sm">
              {deep.beginner.why.map((w, i) => <li key={i}>• {w}</li>)}
            </ul>
          </div>
        )}
      </section>
      ...
      {(deep?.mechanism || deep?.flow) && (
        ...
        >
          ...
          <div className="mt-3 space-y-3">
            {deep.mechanism && (
              <p className="font-mono text-sm leading-relaxed brutal-border bg-volt text-bone p-3">{deep.mechanism}</p>
            )}
            {deep.flow && (
              <div className="space-y-2">
            ...
            {advanced && deep?.advanced?.edgeCases && (
              <div className="brutal-border bg-bone p-3">
                ...
                <ul className="space-y-2 font-mono text-sm">
                  {deep.advanced.edgeCases.map((x, i) => <li key={i}>· {x}</li>)}
                </ul>
              </div>
            )}
            {advanced && deep?.advanced?.engineerNotes && (
              <div className="brutal-border bg-ink text-bone p-3">
                <div className="font-mono text-xs uppercase font-bold mb-1">ENGINEER'S NOTES — why pros choose this</div>
                <ul className="space-y-2 font-mono text-sm">
                  {deep.advanced.engineerNotes.map((x, i) => <li key={i}>⚙ {x}</li>)}
                </ul>
              </div>
            )}
          </div>
      ...
      {deep?.listenFor && (
        <div className="brutal-border bg-sun p-3">
          ...
          <ul className="space-y-1 font-mono text-sm">
            {deep.listenFor.slice(0, advanced ? 99 : 3).map((x, i) => <li key={i}>• {x}</li>)}
          </ul>
        </div>
      )}
          ...
          <ol className="space-y-2 mt-3">
            {deep.walkthrough.map((s, i) => (
              <li key={i} className="brutal-border bg-bone p-2 font-mono text-sm">
                <div><span className="font-bold">{i + 1}. DO:</span> {s.do}</div>
      ...
      {advanced && deep?.proMoves && (
        <details open className="brutal-border bg-ink text-bone p-4">
          ...
          <ul className="space-y-1 mt-2 font-mono text-sm">
            {deep.proMoves.map((x, i) => <li key={i}>★ {x}</li>)}
          </ul>
        </details>
      )}
      ...
      {deep?.mistakes && (
        <details open={advanced} className="brutal-border bg-hot text-bone p-4">
          ...
          <ul className="space-y-1 mt-2 font-mono text-sm">
            {deep.mistakes.map((x, i) => <li key={i}>✗ {x}</li>)}
          </ul>
        </details>
      )}
      ...
      {deep?.workbenchPreset && (
        <Link to="/playground" className="inline-block brutal-border bg-acid px-4 py-2 font-mono text-xs uppercase brutal-press">
          ▶ OPEN IN WORKBENCH
        </Link>
      )}
      ...
      {advanced && deep?.sources && deep.sources.length > 0 && (
        <div className="brutal-border bg-bone p-3">
          <div className="font-mono text-xs uppercase font-bold mb-1">▸ REFERENCES — Ableton Live 12 Reference Manual</div>
          <ul className="font-mono text-xs space-y-1">
            {deep.sources.map((s, i) => <li key={i}>· {s.section} — {s.label}</li>)}
          </ul>
        </div>
      )}
      ...
      {completed && (
        <div className="brutal-border bg-ink text-bone p-4 brutal-shadow flex flex-col md:flex-row items-center justify-between gap-3">
          <div>
            <div className="font-mono text-xs uppercase">MISSION COMPLETE</div>
            <div className="font-display text-2xl">+{m.xp} XP {m.badge ? `· ${m.badge.name}` : ""}</div>
          </div>
          {next ? (
            <Link to="/mission/$slug" params={{ slug: next.slug }} className="brutal-border bg-acid text-ink px-4 py-2 font-display text-lg brutal-press">
              NEXT: {next.title} →
            </Link>
          ) : (
            <Link to="/profile" className="brutal-border bg-acid text-ink px-4 py-2 font-display text-lg brutal-press">VIEW PROFILE →</Link>
          )}
        </div>
      )}
    </div>
