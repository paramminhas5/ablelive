...
export const Route = createFileRoute("/dj/$slug")({
  ...
  notFoundComponent: () => (
    <div className="p-12 font-mono text-center">
      ...
      <div className="mt-2">Mission not found.</div>
      <Link to="/dj" className="brutal-border bg-volt text-bone px-4 py-2 mt-4 inline-block font-mono text-xs uppercase">← DJ Path</Link>
    </div>
  ),
  ...
  return (
    <div className="max-w-2xl mx-auto p-4 md:p-10 space-y-5">
      ...
      <nav className="font-mono text-xs uppercase opacity-50 flex gap-2">
        <Link to="/dj" className="hover:opacity-100">← DJ Path</Link>
        <span>/</span>
      ...
      <header className="brutal-border bg-volt text-bone p-5">
        ...
        {alreadyDone && (
          <div className="mt-2 brutal-border bg-acid text-ink font-mono text-[9px] uppercase px-2 py-1 inline-block">✓ Completed</div>
        )}
      </header>
      ...
      {phase === "learn" && (
        <>
          <section className="space-y-3">
            ...
            {mission.explainer.map((p: string, i: number) => (
          ...
          {mission.practical && mission.practical.length > 0 && (
            <div>
              <button onClick={() => setShowPractical((s) => !s)}
              ...
              {showPractical && (
                <div className="brutal-border mt-2 p-4 bg-sun space-y-3">
                  <div className="font-mono text-[9px] uppercase opacity-60 mb-2">🎛️ IN REKORDBOX</div>
                  {mission.practical.map((step: string, i: number) => (
          ...
          {isAdvanced && mission.advanced && mission.advanced.length > 0 && (
            <div>
              <button onClick={() => setShowAdvanced((s) => !s)}
                ...
                {showAdvanced ? "▲ HIDE ADVANCED" : "▼ ADVANCED DEEP-DIVE"}
              </button>
              {showAdvanced && (
                <div className="brutal-border mt-2 p-4 bg-bone space-y-2">
                  <div className="font-mono text-[9px] uppercase opacity-40 mb-2">🔥 ADVANCED</div>
                  {mission.advanced.map((p: string, i: number) => (
          ...
          <div className="brutal-border border-ink/20 p-3">
            ...
            <div className="flex flex-wrap gap-2">
              {mission.sources.map((s: {label: string; section?: string}, i: number) => (
          ...
          <div className="flex gap-3">
            {prev && (
              <Link to="/dj/$slug" params={{ slug: prev.slug }}
                className="brutal-border bg-bone px-4 py-2 font-mono text-xs uppercase brutal-press">← {prev.title}</Link>
            )}
            {next && (
              <Link to="/dj/$slug" params={{ slug: next.slug }}
                className="ml-auto brutal-border bg-bone px-4 py-2 font-mono text-xs uppercase brutal-press">{next.title} →</Link>
            )}
          </div>
        </>
      )}
      ...
      {phase === "done" && (
        <div className="space-y-4">
          <div className="brutal-border bg-acid p-6 text-center">
            ...
            {mission.badge && (
              <div className="mt-3 brutal-border bg-ink text-bone font-mono text-xs uppercase px-3 py-1 inline-block">🏅 {mission.badge.name}</div>
            )}
          </div>
          <div className="flex gap-3">
            {prev && (
            ...
            {next ? (
              ...
              <Link to="/dj"
                className="flex-1 brutal-border bg-volt text-bone px-4 py-3 font-display text-lg brutal-press text-center">BACK TO DJ PATH</Link>
            )}
          </div>
        </div>
      )}
