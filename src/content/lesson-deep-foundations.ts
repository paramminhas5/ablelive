...
export const FOUNDATIONS_LESSONS: Record<string, LessonDeep> = {
  ...
  "polyrhythm": {
    ...
    advanced: {
      what: [
        "Polyrhythm = two rhythms whose periods are coprime (e.g. 3:2, 4:3, 5:4). They align every LCM beats.",
        "Polymeter = different bar lengths cycling against a common pulse (e.g. 4-bar vs 3-bar loops, aligning every 12 bars).",
        "Tuplets (triplets, quintuplets) inside a single voice are micropolyrhythm relative to the prevailing subdivision.",
      ],
      edgeCases: [
        "DAW grids assume one meter — polymetric work needs manual bar locators or multiple clip lengths in Session view.",
        "Polyrhythms feel disorienting at fast tempos — slow first, build up.",
      ],
      engineerNotes: [
        "Loop length math: LCM of the two cycle counts.",
        "Pan the two voices apart to help the listener separate the layers.",
      ],
    },
    ...
    walkthrough: [
      ...
      { do: "Add a 4-pulse kick.", listen: "Now 4:3:2 — feels endlessly rotating." },
      { do: "Pan congas left and wood block right.", listen: "Both patterns clearly audible as separate streams." },
    ],
    listenFor: ["The downbeat aligning all voices.", "Your perception flipping between hearing 3 and hearing 2.", "Trance / hypnotic feel from long cycles."],
    mistakes: ["Treating triplets as polyrhythm when there's no second steady voice.", "Polyrhythms with no clear downbeat — listener gets lost.", "Loud polyrhythms drowning the main pulse."],
    proMoves: ["Use polyrhythm for percussion only while bass + chords stay 'square'.", "Slow tempo to introduce; speed up to hypnotise.", "Sidechain duck the polyrhythm slightly so the kick still owns the one."],
    quizHard: [
      ...
      { q: "Polymeter differs from polyrhythm because…", options: ["It uses different bar lengths, not different rhythms in the same bar", "It only uses triplets", "It changes tempo", "It is the same thing"], answer: 0 },
    ],
  },
  ...
  "notes-and-octaves": {
    ...
    advanced: {
      what: [
        "12-tone equal temperament (12-TET) divides the octave into 12 equal logarithmic steps. Each semitone = 2^(1/12) ≈ 1.0595 ratio.",
        "MIDI note numbers: C-1 = 0, middle C (C4) = 60, A4 = 69 (440 Hz). One MIDI number per semitone.",
        "Octave equivalence: any frequency f and 2f are perceived as 'same pitch class' — basis of all Western scale theory.",
      ],
      edgeCases: [
        "Different conventions name middle C as C3 or C4 depending on DAW. Live calls it C3; most theory books C4.",
        "Microtonal music subdivides the octave further (e.g., 24-TET quartertones).",
      ],
      engineerNotes: [
        "Frequency of MIDI note n = 440 × 2^((n-69)/12).",
        "Octave choice for layering: same note at different octaves creates depth without harmonic conflict.",
      ],
    },
    ...
    walkthrough: [
      ...
      { do: "Play A4 (440 Hz) and A5 (880 Hz) together.", listen: "Two notes but they fuse — octave equivalence." },
      { do: "Transpose a melody up 12 semitones.", listen: "Identical shape, one octave higher." },
    ],
    listenFor: ["Octave fusion between same-named notes.", "Bass octave choice changing weight without changing notes.", "Higher octaves reading as 'brighter' instantly."],
    mistakes: ["Mixing up sharp and flat naming for the same key.", "Programming bass in the wrong octave (sub-50 Hz disappears on phones).", "Forgetting C3 vs C4 conventions between DAWs."],
    proMoves: ["Layer bass + sub one octave apart for fullness.", "Doubling a melody at the octave thickens it without harmonic ambiguity.", "Use octave shifts for instant arrangement variation."],
    quizHard: [
      { q: "MIDI note 60 is…", options: ["A4", "Middle C", "C0", "C6"], answer: 1 },
  ...
  "pentatonic-scales": {
    ...
    advanced: {
      what: [
        "Major pentatonic = major scale minus 4 and 7 (the half-steps). Minor pentatonic = natural minor minus 2 and 6 (the half-steps).",
        "Blues scale = minor pentatonic + b5 (the 'blue note').",
        "Pentatonics rotate cleanly across all chord changes in a key — single scale, many chords.",
      ],
      edgeCases: [
        "Over a dominant 7 chord, mixolydian pentatonic (1 2 3 5 b7) works better than straight major pentatonic.",
        "Pentatonic over modal jazz changes is the default starter vocabulary.",
      ],
      engineerNotes: [
        "Live's Scale device has pentatonic presets — instant safe-mode soloing.",
        "Use minor pentatonic for hooky melodies in trap/drill (often built around the b3 → 1 motion).",
      ],
    },
    flow: "Take major or minor scale → Remove the two half-step notes → 5 safe notes",
    walkthrough: [
      { do: "Play C major pentatonic (C D E G A).", listen: "Bright and song-like — no clashing notes." },
      { do: "Switch to A minor pentatonic (A C D E G).", listen: "Bluesy, soulful." },
      { do: "Add the blue note (Eb) for blues scale.", listen: "Classic blues phrase territory." },
    ],
    listenFor: ["Absence of half-step tension.", "Hookable, easy-to-sing shapes.", "The signature blue-note bend."],
    mistakes: ["Using major pentatonic over minor-key songs (avoid major 3 against minor 3).", "Sticking only to pentatonic forever — eventually add the missing scale notes for colour.", "Forgetting that pentatonic still has a tonic — phrases want to resolve to 1."],
    proMoves: ["Combine major + minor pentatonic over blues for the BB King sound.", "Use pentatonic for hooks and full scale for fills.", "Pentatonic + rhythmic syncopation = instant memorable melody."],
    quizHard: [
      { q: "Minor pentatonic notes in scale degrees?", options: ["1 2 3 5 6", "1 b3 4 5 b7", "1 b3 5 b7 b9", "1 2 4 5 6"], answer: 1 },
      { q: "Blues scale = minor pentatonic + which note?", options: ["b2", "b5", "#5", "b7"], answer: 1 },
    ],
  },
  ...
  "what-are-chords": {
    ...
    beginner: {
      what: [
        "A chord is two or more notes played simultaneously. The most common chord type is the triad: three notes stacked in 3rds (e.g. C-E-G).",
        "The bottom note (the root) names the chord. The middle note (the 3rd) decides whether it's major or minor. The top note (the 5th) gives it weight.",
        "Almost every song you've heard is built from a sequence of chords. Choose them well and the song writes itself.",
      ],
      why: ["Chords carry emotion — major = bright, minor = dark, diminished = tense.", "Most progressions reuse a handful of patterns across thousands of songs.", "Chords give melody a context that defines its feel."],
    ...
    advanced: {
      what: [
        ...
        "Seventh chords add a 3rd above the triad: maj7, min7, dom7, m7b5, dim7 — each with a distinct flavour.",
        "Voicing = the specific arrangement of notes (octaves, doublings, omissions). Same chord, different voicing, very different sound.",
      ],
      edgeCases: ["Inversions: root, 1st (3rd in bass), 2nd (5th in bass) — change bass line without changing chord function.", "Open vs close voicing: open spreads notes across registers, close stacks them tightly."],
      engineerNotes: ["Avoid doubling the 3rd in major chords (sounds heavy).", "Bass note = perceived root regardless of voicing above.", "Voice leading: move each voice the smallest distance between chords for smoothness."],
    },
    flow: "Root + 3rd + 5th → triad → optional 7th, 9th → voicing decisions",
    walkthrough: [
      { do: "Play C-E-G (C major triad).", listen: "Bright, resolved." },
      { do: "Play C-Eb-G (C minor triad).", listen: "Dark, sad." },
      { do: "Play E-G-C (1st inversion C major).", listen: "Same chord, lighter feel because of E in bass." },
    ],
    listenFor: ["Major vs minor flip by changing one note.", "Inversion changing the bass line without changing chord function.", "7ths adding 'jazz' colour."],
    mistakes: ["Always playing chords in root position — bass becomes static.", "Doubling the 3rd in major triads.", "Skipping voice leading — chords sound disconnected."],
    proMoves: ["Use inversions to make a stepwise bass line under chord progressions.", "Add 7ths to give pop progressions a soulful colour.", "Voice chords differently between verse and chorus for arrangement contrast."],
    quizHard: [
      { q: "A major triad consists of which intervals from the root?", options: ["Maj 3rd + Min 3rd", "Min 3rd + Maj 3rd", "Two Maj 3rds", "Two Min 3rds"], answer: 0 },
  ...
  "daw-explained": {
    ...
    advanced: {
      what: [
        "Core architecture: audio engine + MIDI engine + plugin host + transport + I/O manager + automation system.",
        "Ableton-specific: Session View (clip launcher) vs Arrangement View (timeline) — two views into the same engine.",
        "Plugin formats: VST3, AU (Mac), AAX (Pro Tools). Live also hosts MaxForLive devices.",
      ],
  ...
  "midi-explained": {
    ...
    advanced: {
      what: [
        "MIDI messages: note on/off (with velocity 0-127), pitch bend, control change (CC), program change, channel pressure (aftertouch).",
        "Velocity = how hard a note was struck (0-127). Drives volume and often filter/timbre on synths.",
        "MIDI 2.0 expands to 32-bit resolution, per-note articulation, and improved expression — limited adoption so far.",
      ],
